 ...
 * @inner
 */
agenciesRouter.post("/", async (request: any, response: any, next: any) => {
    try {
 ...
 * @inner
 */
agenciesRouter.get("/me", authenticateToken, (request: any, response: any, next: any) => {
    try {
        ...
        Agency.findById({ _id: decoded.id }, (error: any, result: any) => {
 ...
 */
agenciesRouter.get("/workerIds", authenticateToken, needsToBeAgency, (request: any, response: any, next: any) => {
    try {
 ...
 */
agenciesRouter.get("/workers", authenticateToken, needsToBeAgency, (request: any, response: any, next: any) => {
    try {
        let workerArray: any = [];
        logger.info("Populating array with " + request.agency.users.length + " workers.");
        (Promise as any).map(request.agency.users, (workerId: any) => {
            // Promise.map awaits for returned promises as well.
            return User.findById({ _id: workerId }, (error: any, result: any) => {
                if (!result || error) {
...
agenciesRouter.put("/", authenticateToken, async (request: any, response: any, next: any) => {
    const body = request.body;
    const decoded = response.locals.decoded;
    let passwordHash;
    try {
 ...
 */
agenciesRouter.post("/workers", authenticateToken, needsToBeAgency, (request: any, response: any, next: any) => {
    ...
    try {
        ...
        if (request.body.worker) {
            ...
            if (utils.workerExists(workerId, next)) {
                Agency.findOneAndUpdate({ _id: agencyId }, { $addToSet: { users: [workerId] } }, (error: any, result: any) => {
                    if (error || !result) {
                        return response
                            .status(400)
            ...
            utils.whichWorkersExist(request.body.workers, next, (workerResult: any) => {
                ...
                if (workerIdsToAdd && workerIdsToAdd.length > 0) {
                    ...
                    Agency.findOneAndUpdate({ _id: response.locals.decoded.id }, { $addToSet: { users: workerIdsToAdd } }, (error: any, result: any) => {
                        if (error || !result) {
                            return response
                                .status(400)
 ...
 * @inner
 */
agenciesRouter.get("/businesscontracts", authenticateToken, needsToBeAgency, async (request: any, response: any, next: any) => {
    ...
    try {
        if (contractIds) { // TODO contractIds is not undefined since there is an empty array in db, so code'll get stuck here
            ...
            contractIds.forEach(async (contractId: any, index: any, contractIds: any) => {
 ...
 */
agenciesRouter.get("/:agencyId/businesscontracts", authenticateToken, async (request: any, response: any, next: any) => {
    try {
        const agencyId = request.params.agencyId;
        logger.info("Finding BusinessContracts for Agency " + agencyId);
        Agency.findById(request.params.agencyId, (error: any, agency: any) => {
            if (error || !agency) {
                return response
                    .status(404)
 ...
 */
agenciesRouter.put("/businesscontracts", authenticateToken, needsToBeAgency, async (request: any, response: any, next: any) => {
    try {
        if (request.agency.businessContracts) {
            request.agency.businessContracts.pop();
            request.agency.save((error: any, result: any) => {
                if (error || !result) {
                    return response
                        .status(500)
