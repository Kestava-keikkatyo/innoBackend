...
feelingsRouter.get("/", authenticateToken, needsToBeWorker, async (request, response, next) => {
  try {
    ...
    response.status(200).send(request.worker.feelings.slice(page*limit-limit, page*limit))
  } catch (exception) {
    next(exception)
  }
})
  ...
  try {
    ...
    workerExists(workerId, (worker) => {
      ...
      if (request.agency) {
        ...
        workerExistsInContracts(BusinessContract, contractIds, workerId, (contracts) => {
          // In callback
          for (let i = 0; i < contracts.length; i++) {
            if (contracts[i].user && contracts[i].user.equals(workerId)) {
              if (contracts[i].contractMade) {
                ...
                // Using Array.slice() to paginate feelings.
                return response.status(200).send(worker.feelings.slice(page*limit-limit, page*limit))
              } else {
        ...
        workerExistsInContracts(WorkContract, contractIds, workerId, (contracts) => {
          // In callback
          for (let i = 0; i < contracts.length; i++) {
            if (contracts[i].user && contracts[i].user.equals(workerId)) {
