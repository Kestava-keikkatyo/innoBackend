...
export const sendTextToNotificationsDocument = (req: Request, res: Response, _next: NextFunction) => {
  try {
    ...
    const update:{} = { $addToSet: { message: { text: body.notificationMessage, is_read: false } } }
...
export const textReadNotificationsDocument = (req: Request, res: Response, _next: NextFunction) => {
  try {
    const { params } = req
    const updateFilter:{} = { userId: res.locals.decoded.id, message: { $elemMatch: { _id: params.textId } }  }
    const update:{} = { "message.$.is_read": true }
