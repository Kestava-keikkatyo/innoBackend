 ...
 */
export const postJobVacancyDocument = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    return jobVacancyDocument.save(
      (error: CallbackError, result: IJobVacancyDocument) => {
        if (error) {
        ...
        if (!result) {
          return res
            .status(500)
            .json({ message: "Unable to save job vacancy" });
        }
 ...
 */
const addJobVacancyToAgency = (
  agencyId: string,
  jobVacancy: IJobVacancyDocument,
  res: Response,
  next: NextFunction
) => {
  try {
      ...
      (error: CallbackError, result: IAgencyDocument | null) => {
        if (error || !result) {
          return res
            .status(500)
            .send(
              error || { message: "Received no result when updating user" }
            );
        } else {
 ...
 */
export const getJobVacancyDocuments = (
  _req: Request,
  res: Response,
  next: NextFunction
) => {
 ...
 */
export const getJobVacancyDocumentsForAgency = (
  _req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    JobVacancy.find(
      { agencyId: res.locals.decoded.id },
      (error: CallbackError, docs: IJobVacancyDocument[]) => {
        if (error) {
          return res.status(500).json({ message: error.message });
        }
        if (!docs.length) {
 ...
 * @returns Job vacancy
 */
export const getJobVacancyDocumentById = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    JobVacancy.findById(
      id,
      (error: CallbackError, doc: IJobVacancyDocument | null) => {
        if (error) {
          return res.status(500).json({ message: error.message });
        }
        if (!doc) {
          return res
            .status(404)
            .send({ message: `Could not find job vacancy with ID ${id}` });
        }
 ...
 */
export const updateJobVacancyDocument = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    const jobVacancy: IJobVacancyDocument | null = await JobVacancy.findOne({
      _id: id,
    });
      ...
      (error: CallbackError, updatedDoc: IJobVacancyDocument | null) => {
        ...
        if (!updatedDoc) {
          return res.status(500).send({
            message:
              "Didn't get a result from database while updating job vacancy",
          });
        }
 ...
 * @returns Updated job vacancy
 */
export const deleteJobVacancyDocument = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    JobVacancy.findByIdAndDelete(
      id,
      { lean: true },
      (error: CallbackError, result: IJobVacancyDocument | null) => {
        if (error) {
          return res.status(500).json({ message: error.message });
        }
        ...
        if (!result) {
          return res.status(500).send({
            message:
              "Didn't get a result from database while deleting job vacancy",
          });
        }
        return Agency.findByIdAndUpdate(
          ...
          async (
            error: CallbackError,
            agencyResult: DocumentDefinition<IAgencyDocument> | null
          ) => {
            if (error || !agencyResult) {
              return res.status(500).send(
                error || {
                  message:
                    "Did not receive any result from database when deleting business contract form's id from array",
                }
              );
            }
