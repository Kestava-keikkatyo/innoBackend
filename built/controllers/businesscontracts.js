...
needsToBeAgency, } = require("../utils/middleware");
    ...
    try {
        if (!request.body.role) {
            return response.status(400).json({
                message: "This route requires request to have body.role = agency / business / worker ",
            });
        }
        ...
        if (role.toString() === "agency") {
            ...
            if (contract.agency && contract.agency.toString() === loggedId) {
                ...
                return response.status(401).json({
                    message: "Not authorized to view BusinessContract ID " +
    ...
    try {
        if (request.body.businessId && request.body.workerId) {
            response.status(400).json({
                message: "A BusinessContract can only have EITHER a businessId or a workerId, not both.",
            });
        }
...
const createBusinessContractCallBack = (error, contract, response) => {
    ...
    if (error || !contract) {
        return response.status(500).json({
            error: "Could not save BusinessContract instance to database. Possible error message: " +
                error,
        });
    }
    else {
        ...
        addBusinessContractToParticipants(contract, (error, result) => {
            if (error || !result) {
                ...
                return response.status(500).json({
                    message: "Unable to add created BusinessContract to participants. Possible Error message: " +
                        error.message,
                });
            }
            else {
...
businesscontractsRouter.put("/:businessContractId", authenticateToken, businessContractExists, async (request, response, next) => {
    try {
        ...
        if (business) {
            ...
            if (request.businessContract.business.toString() !==
                request.business._id.toString()) {
                return response.status(401).json({
                    message: "Business with ID " +
                ...
                BusinessContract.findByIdAndUpdate(request.businessContract._id, { contractMade: true }, { new: true }, (error, result) => {
                    if (error || !result) {
                        return response.status(400).send(error || {
                            message: "Could not find and update BusinessContract with ID " +
                        ...
                        response
                            .header({
                            Location: businessContractsApiPath + request.businessContract._id,
                        })
                            .status(200)
...
businesscontractsRouter.delete("/:businessContractId", authenticateToken, businessContractExists, needsToBeAgency, async (request, response, next) => {
    try {
        ...
        if (request.businessContract.agency.toString() !==
            request.agency._id.toString()) {
            logger.info("Agency with ID " +
                request.agency._id +
                " is not authorized to delete BusinessContract with ID " +
                request.businessContract._id +
                ".");
            return response.status(401).json({
                message: "Agency with ID " +
                    request.agency._id +
                    " is not authorized to delete BusinessContract with ID " +
                    request.businessContract._id +
                    ".",
        ...
        if (!success) {
            logger.error("Unable to delete all references to BusinessContract with ID " +
                businessContractId +
                ". Check Agency ID " +
                ...
                ".");
            return response.status(500).json({
                message: "Unable to delete all references to BusinessContract with ID " +
                    businessContractId,
            });
        }
        else {
            BusinessContract.findByIdAndDelete(businessContractId, (error, result) => {
                if (error || !result) {
                    return response.status(500).json({
                        message: "Deleted references to BusinessContract with ID " +
                            businessContractId +
...
const addBusinessContractToParticipants = async (contract, callback) => {
    ...
    if (!agency) {
        ...
        new Error({
            reason: "agency",
            message: "Could not add the BusinessContract to Agency with ID " +
                contract.agency +
                ".",
        }), null);
    }
        ...
        if (contract.user) {
            ...
            if (!worker) {
                ...
                new Error({
                    ...
                    message: "Could not add created BusinessContract to Worker with ID " +
                        contract.user +
                        ".",
                }), null);
            }
            ...
            if (!business) {
                ...
                new Error({
                    ...
                    message: "Could not add created BusinessContract to Business with ID " +
                        contract.business +
                        ".",
                }), null);
            }
